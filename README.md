...
def calculate_user_level(flashcard_sets):
    ...
    if total_attempts == 0:
        return "Unranked"  # No attempts made yet

    overall_percentage = (total_correct / total_attempts) * 100

    # Define levels based on overall percentage
...
def main_menu():
    ...
    while True:  # Main menu loop
        user_level = calculate_user_level(flashcard_sets)  # Calculate the user's level
        ...
        if choice == "1":
            ...
            if set_name in flashcard_sets:
                ...
                # Allow the user to immediately add terms and definitions
                while True:
    ...
    exit()  # Explicitly terminate the program after exiting the loop
    """Main menu for the flashcard program."""
    username, user_data = login()  # Log in or create an account
    flashcard_sets = user_data[username]["flashcard_sets"]  # Load the user's flashcard sets
